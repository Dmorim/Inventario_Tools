...
def config_screen(self, parent):
    ...
    title_frame = ctk.CTkFrame(config, width=190, height=25,
                               border_width=2, border_color='silver', corner_radius=7)
    bottom_frame = ctk.CTkFrame(config, width=190, height=114,
                                border_width=2, border_color='silver', corner_radius=7)
    ...
    confirm_button = ctk.CTkButton(config, text='Confirmar', width=25, height=2,
                                   command=lambda: on_click_confirm(self, config, theme_config_cbb))
    cancel_button = ctk.CTkButton(
        config, text='Cancelar', width=65, height=2, command=lambda: config.destroy())
