def Prod_ZCusto_Screen(Consulta_Screen):    
    ...
    from Consultas.Consultas_ZCusto_List_Screen import List_Treeview_Screen
    
    hub = Consultas_Val_Screen(Consulta_Screen, 'Produtos com Preço de Custo zerado')
    
    val_ven_label = ctk.CTkLabel(hub, text= 'Produtos com Precu = 0:', width= 20, height= 2, font= ('', 16))
    val_ven_text = ctk.CTkLabel(hub, text= prod_get('select count (*) from in01pro where (precu = 0 or (precu is null) or (precu < 0)) and preve > 0 and saldo > 0'), width= 20, height= 2, font= ('', 14))
