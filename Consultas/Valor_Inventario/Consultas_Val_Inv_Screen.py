...
def hub_val_inv(Consulta_Screen, consulta_button, container_manager: ContainerManager):
    try:
        criar_tela_consulta(Consulta_Screen, consulta_button, container_manager,
        ...
        if Consulta_Screen and hasattr(Consulta_Screen, 'after'):
            try:
                Consulta_Screen.after(0, lambda: messagebox.showerror(
                    'Erro', f'Erro ao criar tela de consulta: {e}'))
            except Exception:
                pass
        else:
            try:
